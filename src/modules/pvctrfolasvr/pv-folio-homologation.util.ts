...
export function normalizeEstadoOperativo(value: unknown) {
  ...
  if (estado === 'CERRADOPS' || estado === 'CERRADO PS') return 'CERRADO_PS';
  if (estado.startsWith('CERRADO_PS')) return 'CERRADO_PS';
  ...
  if (!ESTADOS_OPERATIVOS.has(estado)) {
    throw new BadRequestException(
      `ESTA inválido: ${estado}. Valores permitidos: PENDIENTE, PAGADO, CERRADO_PS, MB51PROCES, TRANSMITIR, ANULADO`,
    );
  }
