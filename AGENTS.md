...
- DAT_REC / Proveedores: exponer catálogo activo ordenado y filtrar pedidos por `prov` (ID de `DAT_PROVD`).
- DAT_REC / Jerarquías y rechazo (2026-08-18): proyectar ruta descriptiva JRQ y campos numéricos de jerarquía/graduación en el detalle. Para tipo `RECHAZO`, el Encargado usa `sp_rec_recepcion_rechazar`; excluir `RECHAZADO` de su cola y conservarlo como recepción activa visible para Jefe/Analista.
- DAT_REC / Datos documentales (2026-08-18): mantener guía como texto libre de hasta 100 caracteres, Paquetería hasta 120 y `FOLIO_DOC` hasta 100; el front serializa folios múltiples con separador ` | ` sin requerir tabla nueva.
- DAT_REC / Revisión Jefe (2026-08-18): `PATCH documentos/:docrec/datos` y `PATCH documentos/:docrec/items/:idrec/costo` son exclusivos de `IDROL=2` y `VALIDADO`. El rechazo administrativo debe devolver a sucursal mediante `sp_rec_recepcion_devolver_sucursal`, no reutilizar el rechazo definitivo de mercancía.
- Órdenes de compra / Cancelación: permitir al Jefe de Inventarios cancelar desde `PROCESADO` únicamente cuando la O.C. no tenga recepción registrada ni `CTDREC`; mantener `ABIERTO/PENDIENTE` bajo la regla existente.
- Órdenes de compra / Filtro por Inventarios (2026-08-20): el API conserva el catálogo global de estados; Flutter oculta `PARCIAL` solo para Jefe y Analista.
- Órdenes de compra / Rechazo sincronizado (2026-08-21): `sp_rec_recepcion_rechazar` también cambia `REC_CAB_PED.ESTATUS` a `RECHAZADO`; el catálogo incorpora ese estado. Solo el Jefe puede modificar `CTDPED` o cancelar una O.C. rechazada, sin permitir cambios de costo/unidad ni altas/bajas.
...
- Punto de venta / Pago de Servicios (2026-07-24): el despliegue de `sp_ps_pago_finalize` usa variable `@impd` antes de `sp_executesql`; script `sql/2026-07-24_ps_fix_comprobantes_df01_20260723_vf_0022.sql` actualiza SP, corrige formas y resincroniza entrega.
...
- Inventarios / Signo de salidas MB51 (2026-07-23): `sp_trans_transito` registra el movimiento `121`, `sp_trans_contabilizar` registra el sobrante/descuento origen `124` y `sp_merma_contabilizar` registra la merma con `CTDA/CTOT` negativos; las entradas `123` y reintegraciones `122` permanecen positivas. El script `2026-07-23_mb51_movimientos_historicos_signo_fix.sql` corrige solo los movimientos históricos oficiales con signo positivo, sin modificar `DAT_ART.STOCK`.
...
- Inventarios / Planeacion y sugeridos de compra (2026-07-10): se agrega `SugeridosModule` con ruta `/sugeridos`; usa `DAT_JAA_SUG` para alcance por `USR_MOD_SUC`, calcula con `sp_sugeridos_calcular` aceptando filtros de linea/categoria/marca/tipo de producto, devuelve columnas alineadas al Excel de compra y crea O.C. con `sp_sugeridos_crear_oc` reutilizando `REC_CAB_PED/REC_DET_PED`.
- Inventarios / Planeacion y sugeridos de compra (2026-07-11): mantener la version optimizada de `sp_sugeridos_calcular`; primero filtra `DAT_ART`, luego agrega `DAT_MB51` solo por articulos candidatos y usa `IX_DAT_MB51_SUC_FCND_CLSM_ART` para ventas.
- Inventarios / Recepción de mercancías (2026-08-17): generar `DOCREC` contra `REC_CTRL_DOC_REC`, `DAT_CTR_DOC` y `DAT_MB51`, y usar `IDREC=REC-{DOCREC}-{fila}` como identificador idempotente del movimiento 101.
- Inventarios / Planeacion y sugeridos de compra (2026-07-13): el calculo debe responder paginado con `@PAGE/@LIMIT` y `TOTAL_COUNT`; el front debe pedir 100 registros por pagina para no bloquear la pantalla.
- Inventarios / Ordenes de compra (2026-07-16): `SugeridosService` debe permitir alcance de sucursal para `DAT_JAA_SUG` y `DAT_ORD_COMP`, porque `/sugeridos` alimenta calculo y listado de O.C.
...
- Ordenes de trabajo / Cambio material y Merma (2026-07-24): cada ORD derivada debe persistir snapshot propio `PVTA_UNITARIO/PVTAT_BASE`; diferencia sellada usa fracción `CTD_C_M`, catálogo actual para nueva ORD e IVA según `DAT_SUC`/folio. Misma ART está permitida.
