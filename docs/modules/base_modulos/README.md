...
- facturación paginación (2026-03-31, actualizado 2026-06-29): el pageSize por defecto y máximo en `/facturacion/pendientes` queda en 150 registros para alinear frontend y backend.
...
- selección masiva por IDFOL (2026-06-29): `POST /facturacion/pendientes/idfols` acepta `idFols` (máx. 500) y devuelve una página única con todos los folios `PENDIENTE` encontrados, respetando control de sucursal para usuarios sin gestión; no requiere SP adicional.
...
- edición fiscal por folio (2026-07-30): `PATCH /facturacion/:idFol/cliente-fiscal` actualiza el cliente resuelto desde un folio `PENDIENTE` con permiso de gestión `FACTURA`; conserva `SUC` y evita aplicar por error el alcance del mantenimiento general de clientes.
