...
CLIEN),
    ...
    ORDER BY CASE WHEN IDFOL = @idfolNorm THEN 0 ELSE 1 END, FCN DE
SC, FCNM DESC;
...
dfolActual;
    ...
    SET FORM = CASE
      ...
      WHEN FORM_RAW IN ('TARJETA CREDITO', 'TARJETA DE CREDITO') THEN 'TARJETA CREDITO'
    ...
    BEGIN
      ...
      IF (SELECT COUNT(DISTINCT FORM) FROM @FORMAS) >
 1
      ...
      WHERE F.FORM IN ('TARJETA', 'CHEQUE', 'TRANSFERENCIA', 'DEPOSITO 3RO')
        ...
        AND NOT EXISTS (
          ...
          WHERE UPPER(LTRIM(RTRIM(ISNULL(R.IDREF, '')))) = UPPER(LTRIM(RTRIM(ISNULL(F.AUT, ''))))
            ...
            AND UPPER(LTRI
M(RTRIM(ISNULL(R.ESTATUS, '')))) = 'PROCESADO'
            AND (
              NULLIF(UPPER(LTRIM(RTRIM(ISNULL(R.TIPO, '')))), '') IS NULL
              OR UPPER(LTRIM(RTRIM(ISNULL(R.TIPO, '')))) = F.FORM
            )
            AND NULLIF(LTRIM(RTRIM(I
SNULL(R.RfcEmisor, ''))), '') IS NOT NULL
...
SELECT 1
    ...
    BEGIN
      ...
      SET @ejercido = CASE WHEN @ejercido < 0 THEN ABS(@ej
ercido) ELSE 0 END;
...
object_id = @folFormObjId;
    ...
    IF
 ISNULL(LTRIM(RTRIM(@idfolVisibleNuevo)), '') = ''
    ...
    SET @sql = N'DELETE FROM ' + @folFormTable + 
N' WHERE IDFOL = @pIDFOL';
...
rm,
    ...
    IF OBJECT_ID('dbo.REF_DETALLE') IS
 NOT NULL
    BEGIN
    ...
    SELECT
      @headHasREQF = MAX(CASE WHEN UPPER(name) = 'REQF' TH
EN 1 ELSE 0 END),
      ...
      @he
adHasOPVM = MAX(CASE WHEN UPPER(name) = 'OPVM' THEN 1 ELSE 0 END),
      ...
      @headHasORIGENAU
T = MAX(CASE WHEN UPPER(name) = 'ORIGEN_AUT' THEN 1 ELSE 0 END)
    FROM sys.columns
...
TOTAL' +
        ...
        CASE WHEN 
@headHasFCNM = 1 THEN N', FCNM = @pNOW' ELSE N'' END +
        CASE WHEN @headHasOPVM = 1 THEN N', OPVM = @pOPV' ELSE N'' END +
        CASE WHEN @headHasTRA = 1 THEN N', TRA = @pTRA' ELSE N'' END +
        CASE WHEN @headHasIDFOLINICIAL = 1 THEN N', IDFO
LINICIAL = @pIDFOLINICIAL' ELSE N'' END +
        CASE WHEN @headHasORIGENAUT = 1 THEN N', ORIGEN_AUT = @pORIGENAUT' ELSE N'' END + N'
      WHERE IDFOL = @pIDFOLOLD';
    SET @execRqfac = CASE WHEN @tipotranNorm = 'CA' THEN 0 ELSE CASE WHEN @RQFAC = 1 TH
EN 1 ELSE 0 END END;

    EXEC sys.sp_executesql
      @sql,
      N'@pIDFOLNEW NVARCHAR(255), @pTOTAL MONEY, @pRQFAC INT, @pTIPOTRAN NVARCHAR(10), @pOPV NVARCHAR(255), @pTRA NVARCHAR(255), @pIDFOLINICIAL NVARCHAR(255), @pORIGENAUT VARCHAR(2), @pIDFOLOLD 
NVARCHAR(255), @pNOW DATETIME',
    ...
    BEGIN
      IF OBJECT_ID('dbo.sp_fact_sync_folio_vf') IS NULL
        THROW 51033, 'No existe dbo.sp_fact_sync_folio_vf. 
Ejecute sql/sp_fact_sync_folio_vf_create.sql', 1;
