  ...
  WHERE LTRIM(RTRIM(ISNULL(value, ''))) <> '';
    ...
    WITH base AS (
      ...
      o.ESTATUS,
        ...
        ROW_NUMBER() OVER (ORDER BY ISNULL(o.FCNMOD, o.FCNM) DESC, o.IORD DESC) AS RN,
    ...
    WHERE
      ...
      AND (@LABOR IS NULL OR CAST(ISNULL(o.LABOR, '') AS NVARCHAR(255)) = @LABOR)
        ...
        AND (@ESTSEGU IS NULL OR CAST(ISNULL(o.ESTSEGU, '') AS NVARCHAR(255)) = @ESTSEGU)
...
BEGIN
  ...
  BEGIN TRY
    ...
    SET
      ...
      ESTATUS = CASE WHEN @remanente > 0 THEN ESTATUS ELSE 4 END,
      FCNMOD = GETDATE(),
      COMAD = LEFT(
        CONCAT(
          ISNULL(CAST(COMAD AS NVARCHAR(MAX)), ''),
