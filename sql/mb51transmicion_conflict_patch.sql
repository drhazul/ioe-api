/* mb51transmicion_conflict_patch.sql
   Alternativa de procedimiento para prevenir nuevos -U y registrar conflictos IDPD/DOCP.
   NO ejecutar en producción sin ventana de mantenimiento.
*/
USE IOELOCAL;
SET NOCOUNT ON;
SET XACT_ABORT ON;

IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name='MB51_CONFLICT_LOG' AND schema_id=SCHEMA_ID('dbo'))
BEGIN
  CREATE TABLE dbo.MB51_CONFLICT_LOG (
    ID BIGINT IDENTITY(1,1) PRIMARY KEY,
    IDPD NVARCHAR(510) NOT NULL,
    DOCP_EXISTE NVARCHAR(255) NULL,
    DOCP_NUEVO NVARCHAR(255) NULL,
    ...
    SRC NVARCHAR(40) NULL
  );
END;
GO

ALTER PROCEDURE dbo.sp_mb51_transmitir_folio
  @IDFOL NVARCHAR(255),
  @USER NVARCHAR(255) = NULL
AS
BEGIN
