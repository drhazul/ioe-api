/*
  2026-03-25
  Ajustes ORDs:
  - Admin y jefe de taller visualizan todo el panel operativo dentro de sus sucursales autorizadas.
  - Analista y encargados de maquila/biselado aplican visibilidad adicional por laboratorio asignado.
  - Encargados de maquila/biselado solo visualizan ORDs de su respectivo TIPO.
  - Se agrega anulación masiva a ESTSEGU = 4.
*/
...
CREATE OR ALTER PROCEDURE dbo.sp_ordenes_trabajo_anular_lote
  @IORDS NVARCHAR(MAX),
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM @ords)
    THROW 58350, 'Debe enviar al menos una ORD en @IORDS', 1;
  ...
  BEGIN
    ...
    THROW 58351, @missingMsg, 1;
  END;

  IF EXISTS (
    SELECT 1
    FROM @target t
    WHERE ISNULL(t.ESTSEGU, -1) = 4
  )
  BEGIN
    ...
    DECLARE @invalidMsg NVARCHAR(2048) = LEFT(
      CONCAT('Las siguientes ORDs ya están anuladas: ', ISNULL(@invalidIords, '')),
      2048
    );
    THROW 58352, @invalidMsg, 1;
  END;
