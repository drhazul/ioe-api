/*
  Reporte comercial por cliente - Version 2
...
/* Columnas: SUC, MES, AÑO, CLIENTE, NOMBRE DEL CLIENTE, CANTIDAD DE FOLIOS, */
/*           EFECTIVO, TARJETA, TRANSFERENCIA, DEPOSITO 3RO, CREDITO, CHEQUE */
/* -------------------------------------------------------------------------- */
WITH base_formas AS (
  SELECT
...
/* Columnas: SUC, CLIEN, cliente, clasificacion                               */
/* -------------------------------------------------------------------------- */
WITH base_fact AS (
  SELECT
    UPPER(LTRIM(RTRIM(ISNULL(a.SUC, 'SIN_SUC')))) AS SUC,
...
/* -------------------------------------------------------------------------- */
/* Hoja: TopCotAbiertas                                                       */
/* Columnas: suc, CLIEN, cliente, cotizaciones_abiertas,                      */
/*           importe_cotizaciones_abiertas                                    */
/* Considera contrapartes de devolución: DA/DP                                */
/* -------------------------------------------------------------------------- */
WITH base_cot AS (
  SELECT
    UPPER(LTRIM(RTRIM(ISNULL(a.SUC, 'SIN_SUC')))) AS SUC,
  ...
  WHERE CAST(a.FCNM AS DATE) BETWEEN @fecha_inicio AND @fecha_fin
    AND UPPER(LTRIM(RTRIM(ISNULL(a.ESTA, '')))) NOT IN ('ANULADA', 'ANULADO', 'VA')
    AND UPPER(LTRIM(RTRIM(ISNULL(a.AUT, '')))) IN ('CP', 'CPF', 'DA', 'DP')
)
SELECT TOP 30
  SUC AS suc,
  CLIEN,
  cliente,
  SUM(CASE WHEN AUT IN ('CP', 'CPF') THEN 1 WHEN AUT IN ('DA', 'DP') THEN -1 ELSE 0 END) AS cotizaciones_abiertas,
  SUM(CASE WHEN AUT IN ('CP', 'CPF') THEN ABS(IMPT) WHEN AUT IN ('DA', 'DP') THEN -ABS(IMPT) ELSE 0 END) AS importe_cotizaciones_abiertas
FROM base_cot
GROUP BY SUC, CLIEN, cliente
...
/* -------------------------------------------------------------------------- */
/* Hoja: TopVentasNetas                                                       */
/* Columnas: suc, CLIEN, cliente, cotizaciones_VF, importe_VF                 */
/* Solo AUT VF y su contraparte DVF                                           */
/* -------------------------------------------------------------------------- */
WITH base_vf AS (
  SELECT
    UPPER(LTRIM(RTRIM(ISNULL(a.SUC, 'SIN_SUC')))) AS SUC,
  ...
  WHERE CAST(a.FCNM AS DATE) BETWEEN @fecha_inicio AND @fecha_fin
    AND UPPER(LTRIM(RTRIM(ISNULL(a.ESTA, '')))) NOT IN ('ANULADA', 'ANULADO', 'VA')
    AND UPPER(LTRIM(RTRIM(ISNULL(a.AUT, '')))) IN ('VF', 'DVF')
)
SELECT TOP 30
  SUC AS suc,
  CLIEN,
  cliente,
  SUM(CASE WHEN AUT = 'VF' THEN 1 WHEN AUT = 'DVF' THEN -1 ELSE 0 END) AS cotizaciones_VF,
  SUM(CASE WHEN AUT = 'VF' THEN ABS(IMPT) WHEN AUT = 'DVF' THEN -ABS(IMPT) ELSE 0 END) AS importe_VF
FROM base_vf
GROUP BY SUC, CLIEN, cliente
ORDER BY importe_VF DESC, cotizaciones_VF DESC;

/* -------------------------------------------------------------------------- */
/* Hoja: CreditosMesForma                                                     */
/* Columnas: SUC, MES, AÑO, IMPORTE CREDITO, TRANSACCIONES CREDITO,          */
/*           EFECTIVO, TARJETA, TRANSFERENCIA, DEPOSITO 3RO, CHEQUE           */
/* -------------------------------------------------------------------------- */
